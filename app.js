...
function getBrasilBroadcast(broadcast) {
    if (!broadcast || !broadcast.day || !broadcast.time) return "Horário não disponível";
...
async function openAnimeDetails(id) {
    ...
    document.getElementById('m-release-date').innerText = anime.broadcast.string || "N/A";
    document.getElementById('m-broadcast').innerText = getBrasilBroadcast(anime.broadcast);
